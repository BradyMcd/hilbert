...
/* hil_rotation
 *  Rotates distance by 90 degrees clockwise where that distance is along a curve
 *  generated by nBits levels of recursion
 *
 *  Thanks to the patterns left by the recursive generation of a hilbert curve we
 *  need only check the second most significant bit and either flip every bit or
 *  flip only the oddly indexed bits
 */
uint64_t hil_rotation( uint8_t recursions, uint64_t distance ){
  implementation_precond( recursions );
  assert( (uint64_t)1<<(recursions*2) > distance
          /*The distance must be less than the side length squared*/ );
  const uint64_t ODD_BITS = 0x5555555555555555;
  const uint64_t ALL_BITS = 0xFFFFFFFFFFFFFFFF;

  //The second most significant bit will tell us what bits must flip
  uint64_t check_bit = (uint64_t)1<<( recursions * 2 - 1 );

  uint64_t mask = ( distance & check_bit ) ?
    ALL_BITS >> ( 64 - ( recursions*2 ) ): ODD_BITS >> ( 64 - ( recursions*2 ) );
